...
class CommentOnPullRequestService {
  ...
  private async createReviewComments(files: FilenameWithPatch[]) {
    ...
    // Add overall review summary for traditional mode
    await this.createTraditionalOverallReview(files, suggestionsByFile);
  }

  private async createTraditionalOverallReview(files: FilenameWithPatch[], suggestionsByFile: any[]) {
    console.log('📋 Creating overall review summary for traditional mode');
    
    const totalFiles = files.length;
    const reviewedFiles = suggestionsByFile.length;
    const hasIssues = reviewedFiles > 0;

    const overallComment = `## 🤖 ChatGPT Code Review Summary

📊 **Files Reviewed:** ${reviewedFiles} of ${totalFiles} files
${hasIssues ? '⚠️' : '✅'} **Status:** ${hasIssues ? 'Issues found requiring attention' : 'No major issues detected'}

### 📝 Review Details
${hasIssues 
  ? `- Found ${reviewedFiles} file(s) with suggestions for improvement
- Please review the individual file comments above
- Consider addressing the feedback before merging` 
  : `- All reviewed files appear to follow good practices
- No significant issues detected in the code changes
- Code looks good for merging`}

---
*This review was generated by ChatGPT Code Reviewer v1.5.0*`;

    try {
      await this.createSimplePRComment(overallComment);
      console.log('✅ Successfully created traditional mode overall review comment');
    } catch (error) {
      console.error('❌ Failed to create traditional mode overall review comment:', error);
    }
  }
